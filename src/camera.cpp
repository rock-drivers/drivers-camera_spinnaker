...
void Camera::setFrameRate(const float frame_rate)
{
  ...
  LOG_DEBUG_S<<"Minimum Frame Rate: \t " << ptrAcquisitionFrameRate->GetMin();
  LOG_DEBUG_S<<"Maximum Frame rate: \t " << ptrAcquisitionFrameRate->GetMax();

  // Finally Set the Frame Rate
  setProperty(node_map_, "AcquisitionFrameRate", frame_rate);

  LOG_DEBUG_S<<"Current Frame rate: \t " << ptrAcquisitionFrameRate->GetValue();
}
...
void Camera::setImageControlFormats(const camera_spinnaker::SpinnakerConfig& config)
{
